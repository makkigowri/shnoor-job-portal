            ...
            {!loading && data.jobs.map((job) => (
              <tr key={job.id} className="border-t border-gray-100">
                ...
                <td className="px-6 py-3">
                  <ActionMenu
                    items={[
                      { key: "view", label: "View", onClick: () => handleView(job.id) },
                      {
                        key: "export",
                        label: exportingApplicantsId === job.id ? "Exporting..." : "Export",
                        disabled: exportingApplicantsId === job.id,
                        onClick: () => handleExportApplicants(job.id)
                      },
                      {
                        key: "toggle-status",
                        label: job.status === "Active" ? "Close Job" : "Activate Job",
                        onClick: () =>
                          setConfirmAction({
