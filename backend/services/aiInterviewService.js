...
const { generateInterviewQuestion, evaluateInterviewTranscript } = require("./ollamaService");
...
const finalizeInterview = async (interview, questions) => {
  const evaluation = await evaluateInterviewTranscript({
    jobRole: interview.job_role,
    skills: interview.candidate_skills,
    experience: interview.candidate_experience,
    assessmentPercentage: interview.assessment_percentage,
    exchanges: questions.map((q) => ({ question: q.question_text, answer: q.candidate_answer }))
  });
  ...
  const updatedInterview = await aiInterviewModel.finalizeInterview(interview.id, {
    technicalScore: evaluation.technicalKnowledge,
